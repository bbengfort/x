...
func NewFixedInterval(delay time.Duration, etype events.Type, echan chan<- error) *FixedInterval {
	...
	interval.Init(delay, etype, echan)
	return interval
}
...
func NewRandomInterval(minDelay, maxDelay time.Duration, etype events.Type, echan chan<- error) *RandomInterval {
	interval := new(RandomInterval)
	interval.Init(minDelay, maxDelay, etype, echan)
	return interval
}
...
type FixedInterval struct {
	...
	etype       events.Type   // The type of event dispatched by the timer
	echan       chan<- error  // Channel to send errors on
...
func (t *FixedInterval) action() {
	...
	if err := t.Dispatcher.Dispatch(t.etype, nil); err != nil {
		t.echan <- err
		return
	}
...
// returns true if timer is running (not thread-safe).
func (t *FixedInterval) running() bool {
	return t.timer != nil
}
...
func (t *FixedInterval) Register(callback events.Callback) {
	t.Dispatcher.Register(t.etype, callback)
}
...
func (t *RandomInterval) Init(minDelay, maxDelay time.Duration, etype events.Type, echan chan<- error) {
	t.minDelay = int64(minDelay)
	t.maxDelay = int64(maxDelay)
	t.FixedInterval.Init(0, etype, echan)
}
